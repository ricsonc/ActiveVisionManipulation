...
def train(
        env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor,
        ...
        batch_size, memory, single_train, 
        tau=0.05, eval_env=None, param_noise_adaption_interval=50,
        **kwargs):
    ...
    agent = DDPG(actor, critic, memory, env.observation_space.shape, env.action_space.shape, single_train,
    ...
    with U.single_threaded_session() as sess:
        ...
        for epoch in range(global_t, nb_epochs):
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                rollout_steps = int(nb_rollout_steps/MPI.COMM_WORLD.Get_size())
                for t_rollout in range(rollout_steps):
                    ...
                    if done:
                        ...
                        if kwargs["her"]:
                            ...
                            her_states, her_rewards = env.apply_hindsight(states, actions, copy_obs(new_obs))
                ...
                if (eval_env is not None) and rank==0:
                    ...
                    if(eval_info["done"]=="goal reached"):
                        logger.info(
                            "success, training epoch:%d,starting config:"%epoch,
                            allow_nice_printing(eval_obs_start),
                            'final state',
                            allow_nice_printing(eval_obs)
                        )
