...
class DDPG(object):
    def __init__(
        self, actor, critic, memory, observation_shape, action_shape, single_train,
        param_noise=None, action_noise=None, gamma=0.99, tau=0.001, normalize_returns=False,
        enable_popart=False, normalize_observations=True, batch_size=128,
        ...
        self.tuple_obs = (isinstance(observation_shape[0], list) or
                          isinstance(observation_shape[0], tuple))

        if self.tuple_obs:
            self.obs0 = [
                tf.placeholder(tf.float32, shape=(None,) + observation_shape_, name='obs0')
                for observation_shape_ in observation_shape
            ]
            self.obs1 = [
                tf.placeholder(tf.float32, shape=(None,) + observation_shape_, name='obs1')
                for observation_shape_ in observation_shape
            ]
        else:
        ...
        if self.normalize_observations:
            with tf.variable_scope('obs_rms'):
                obs_shape = observation_shape[0] if self.tuple_obs else observation_shape
    ...
    def store_transition_to_memory(self, obs0, action, reward, obs1, terminal1):
    ...
    def train(self, summary_var):
        ...
        if self.normalize_returns and self.enable_popart:
            ...
            old_mean, old_std, target_Q = self.sess.run(
                [self.ret_rms.mean, self.ret_rms.std, self.target_Q], feed_dict=fd
            )
        ...
        current_summary, actor_grads, actor_loss, critic_grads, critic_loss, _ = self.sess.run(ops, feed_dict=fd)
